...
/**
 * Prints tables in plain ascii.
 * 
 */
public class AsciiPrinter implements PrinterSPI {
  ...
  public String print(TablePrinter table) {
    ...
    if (table != null && table.getColumnHeaders().size() > 0) {
      ...
      if (table.getRows().size() > 0) {
        ...
        int cellPadding = lineSeparator.length() + 1 + 1;
        printedTable.append(prepareCell(EMPTY_VALUE, cellPadding - EMPTY_VALUE.length(),
            Alignment.LEFT));
